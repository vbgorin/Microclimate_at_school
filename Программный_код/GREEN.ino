...
//Облако VizIoT
const String VizIoT_Device_key = "MXC00KVGZVG7LW3W"; //Ключ VizIoT
const String VizIoT_Device_pass = "K9SHE67EA029SQRDPE9R"; //Пароль VizIoT
...
BH1750 lightMeter;
// Объявление переменныx и констант
int ensStatus, AQI, TVOC, CO2, getFlags, error, light;
float temperature, humidity, hi, p10, p25;
float tk = 4.8; //калибровочный коэффицент датчика температуры
int hk = 5; //калибровочный коэффициент датчика влажности
#define PIN D5 // Светодиод подключен к контакту D5
/*
Настраиваем библиотеку Adafruit_NeoPixel:
...
void setup() {
    ...
    // Компенсация температуры и влажности датчика газа. 
...
void loop() {
    ...
    myOLED.print("Освещенность", 0, 6);
    myOLED.print(lux, 80, 6);
    myOLED.print("lx", 115, 6);
...
*/
    light = map(lux, 0, 700, 10, 255); //рассчет уровня яркости светодиодов
    ...
    VizIoT(serverURL, VizIoT_Device_key, VizIoT_Device_pass, temperature, hi, humidity, AQI, TVOC, CO2, getFlags, rssi, p25, p10, lux); // Отправка данных в облако VizIoT
    delay(10000);
}
/*-------------БЛОК ФУНКЦИЙ-----------------------------------*/
...
void VizIoT(String serverURL, String VizIoT_Device_key, String VizIoT_Device_pass, float temperature, float hi, float humidity, int AQI, int TVOC, int CO2, int getFlags, long rssi, float p25, float p10, int lux) {
    String url = serverURL + "?key=" + VizIoT_Device_key + "&pass=" + VizIoT_Device_pass + "&Temp=" + String(temperature) + "&Hic=" + String(hi) + "&Hum=" + String(humidity) + "&AQI=" + String(AQI) + "&TVOC=" + String(TVOC) + "&CO2=" + String(CO2) + "&Flag=" + String(getFlags) + "&RSSI=" + String(rssi) + "&p2.5=" + String(p25) + "&p10=" + String(p10) + "&lux=" + String(lux);
